...
!!! note
    The function requires the stdlib Test to be loaded (e.g. `using Test`), as its functionality
    is implemented via a package extension.
...
"""
function test_explicit_imports(package::Module, file=pathof(package); kw...)
    # if we have the extension, forward to `_test_explicit_imports`
    if applicable(_test_explicit_imports, package, file)
        return _test_explicit_imports(package, file; kw...)
    else
        error("Must load the Test stdlib (e.g. `using Test`) first in order to use `test_explicit_imports`.")
    end
end

function _test_explicit_imports end
